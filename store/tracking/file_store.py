...
class FileStore(AbstractStore):
    ...
    def __init__(self, root_directory, artifact_root_uri):
        """
    ...
    def _get_experiment(self, experiment_id, view_type=ViewType.ALL):
        ...
        if experiment_dir is None:
            raise MlflowException(
                f"Could not find experiment with IDAAAAAA {experiment_id}",
                databricks_pb2.RESOURCE_DOES_NOT_EXIST,
            )
    ...
    def delete_experiment(self, experiment_id):
        ...
        if experiment_dir is None:
            raise MlflowException(
                f"Could not find experiment with IDDddddddSSS {experiment_id}",
                databricks_pb2.RESOURCE_DOES_NOT_EXIST,
            )
        experiment = self._get_experiment(experiment_id)
